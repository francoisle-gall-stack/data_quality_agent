...
def build_investigation_context() -> InvestigationContext:
    ...
    failed_node = get_failed_node(diagnostic)
    selected_manifest = load_filtered_manifest(target_dir, failed_node["unique_id"])
    compiled_sql = load_compiled_sql(
        selected_manifest,
        dbt_dir=settings.dbt_dir,
        project_root=PROJECT_ROOT,
    )
    git = load_git_history(
        failed_node.get("file_path", ""),
        project_root=PROJECT_ROOT,
    )
