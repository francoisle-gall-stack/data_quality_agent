...
export function Dashboard() {
  ...
  const refreshDate = filters.data_refresh_date ? String(filters.data_refresh_date).slice(0, 10) : null;
  return <div className="layout"><FilterSidebar filters={filters} query={query} setQuery={setQuery} /><main className="content"><header className="page-header"><div><span className="eyebrow">Vue d’ensemble</span><h1>Retail Analytics</h1><p>Explorez la performance commerciale et interrogez vos données.</p></div><div className="header-actions"><a className="pipeline-link" href="/pipeline">Lancer le pipeline dbt <span>→</span></a><div className="header-meta"><span className="live-dot" /><span>{refreshDate ? `Données au ${new Date(`${refreshDate}T00:00:00`).toLocaleDateString("fr-FR")}` : "Date indisponible"}</span></div></div></header><KpiCards data={kpis} /><div className="section-heading"><div><span className="eyebrow">Performance</span><h2>Indicateurs clés</h2></div><span className="muted">Sur la période sélectionnée</span></div><div className="dashboard-workspace"><div className="grid"><ChartPanel title="CA quotidien" data={daily} x="order_date" y="revenue" /><ChartPanel title="Commandes quotidiennes" data={daily} x="order_date" y="orders" /><ChartPanel title="CA par pays" data={country} x="country_code" y="revenue" kind="bar" /><ChartPanel title="CA par canal" data={channel} x="channel" y="revenue" kind="bar" /></div><ChatPanel query={query} /></div></main></div>;
}
